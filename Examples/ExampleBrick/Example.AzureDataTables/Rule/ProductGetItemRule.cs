            ...
            try
            {
                if (context.Object is ApiGetItemAfterEvent<ProductDto> de)
                {
                    ...
                    var query = QueryBuilder.New()
                        .IsEqual(nameof(ProductCategory.PartitionKey), item.StorageKey)
                        .Build();
                    var respQ = await _productCategoryRepo.QueryAsync(query);
                    if (respQ.Success && respQ.List.Count > 0)
                    {
                        var categories = respQ.List.Select(x => x.RowKey).ToArray();
                        query = QueryBuilder.New()
                            .IsInSet(nameof(Category.PartitionKey), categories)
                            .Build();
